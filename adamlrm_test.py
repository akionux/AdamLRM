...
class AdamLRMOptimizerTest(test.TestCase):
  ...
  def doTestBasic(self, use_callable_params=False):
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.cached_session(use_gpu=True):
        ...
        opt = adamlrm.AdamLRM(
            learning_rate=learning_rate, lr_multiplier={'var1':1e-2})
  ...
  def testBasicWithAmsgrad(self):
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.cached_session(use_gpu=True):
        ...
        opt = adamlrm.AdamLRM(amsgrad=True, lr_multiplier={'var1':1e-2})
        if not context.executing_eagerly():
          update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
      ...
      with self.cached_session(use_gpu=True):
        ...
        for t in range(3):
          ...
          var1_np, m1, v1 = adam_update_numpy(
              var1_np, grads1_np, t, m1, v1, lr=1e-2*lr_np)
  ...
  def testTensorLearningRate(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with self.cached_session(use_gpu=True):
        ...
        grads0 = constant_op.constant(grads0_np)
        grads1 = constant_op.constant(grads1_np)
        opt = adamlrm.AdamLRM(constant_op.constant(0.001),
            lr_multiplier={'var1':1e-2})
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        variables.global_variables_initializer().run()
